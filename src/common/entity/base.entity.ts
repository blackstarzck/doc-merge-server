...
export abstract class BaseModel {
  ...
  @Transform(({ obj }) => obj.win_price ?? safeFloor(multiply(obj.bk_price, obj.win_rate)))
  ...
  @Transform(({ obj }) => obj.bk_supply_price ?? safeFloor(multiply(obj.bk_price, obj.bk_supply_rate)))
  ...
  @Transform(({ obj }) => obj.purchase_cost ?? multiply(obj.bk_price, obj.bk_cost_late))
  ...
  @Transform(({ obj }) => obj.org_m_price ?? multiply(obj.total_bks, obj.org_m_per_price))
  ...
  @Transform(({ obj }) => obj.m_supply_total_price ?? multiply(obj.total_bks, obj.m_supply_price))
  ...
  @Transform(({ obj }) => obj.final_delivery_quantity ?? subtract(obj.total_bks, obj.out_of_stock_bks))
  @Column({ type: 'int', comment: '최종납품권수', nullable: true })
  final_delivery_quantity: number // total_bks - out_of_stock_bks

  @Transform(({ obj }) => obj.m_final_sales ?? multiply(obj.m_supply_price, obj.m_supply_price))
  ...
  @Transform(({ obj }) => obj.final_delivery_price ?? subtract(obj.bk_price, obj.out_of_stock_bks))
  @Column({ type: 'int', comment: '최종 납품정가', nullable: true })
  final_delivery_price: number // bk_price - out_of_stock_bks

  @Transform(({ obj }) => obj.final_bk_sales ?? multiply(obj.bk_supply_rate, obj.m_supply_price))
  @Column({ type: 'int', comment: '최종 도서매출액', nullable: true })
  final_bk_sales: number // bk_supply_rate * m_supply_price

  @Transform(({ obj }) =>
    obj.bk_revenue ?? subtract(obj.final_bk_sales, multiply(obj.m_supply_price, obj.bk_cost_late)),
  )
  ...
  @Transform(({ obj }) => {
    ...
    return obj.d_day ?? Math.floor((deliveryDate - todayDate) / (1000 * 60 * 60 * 24)); // 일(day) 단위 차이
  })
